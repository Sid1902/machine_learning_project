...
def evaluate_regression_model(model_list : list , X_train : np.ndarray , y_train : np.ndarray,X_test : np.ndarray,y_test:np.ndarray , base_accuracy : float = 0.6) ->MetricInfoArtifact :
    ...
    try :  
        ...
        for model in model_list :
            ...
            # print(class_name)  # Output: LinearRegression
...
class ModelFactory :
    ...
    def update_property_of_class(instance_ref : object , property_data : dict) :
        try:
            #  Checking whether the list of params recieved is dict or not 
            if not isinstance(property_data,dict) :
                raise Exception("property_data paramter required to be dictionary")
            print(property_data)

            for key,value in property_data.items():
                logging.info(f"Executing:$ {str(instance_ref)}.{key}={value}")

                # eg : setattr(lr,'fit_intercept',True) 
    ...
    def class_for_name (module_name :str , class_name :str):
        """
        This functions imports the module and class mentioned in model.yaml file
        eg. from sklearn.linear_model import LinearRegerssion()
        """
        try: 

            #  importlib imports the module name specified eg. sklearn.linear_model
    ...
    def execute_grid_search_operation(self,initialized_model : InitializedModelDetail,input_feature,
        ...
        """
        try:
            #  Creating instance of GridSearch class 

            grid_search_cv_ref = ModelFactory.class_for_name(module_name=self.grid_search_cv_module,
                                                             class_name=self.grid_search_class_name)
            
            #  output for above line would be : GridSearchCV()

            #  now updating the params of GridSearchCV class
    ...
    def get_initialized_model_list(self) -> List[InitializedModelDetail] :
        ...
        try:
            ...
            # models_initialization_config.keys() refers to model selection key in "model.yaml"
            # model_serial_number refers to serial number i.e module_0 module_1 in "model.yaml"
            for model_serial_number in self.models_initialization_config.keys():
                
                # getting the class_names , module_names related to module_0 module_1 etc in "model.yaml"
                ...
                model = model_obj_ref() #initialized the model 

                if PARAM_KEY in model_initialization_config :

                    #  checking in "model.yaml" if there is any paramters for the models initialized  
                    model_obj_property_data = dict(model_initialization_config[PARAM_KEY])

                    # updating the params present in file to model i.e LinearRegression() -> LinearRegression(fit_intercept = True)

                    model = ModelFactory.update_property_of_class(instance_ref = model,
                                                                  property_data = model_obj_property_data)
                    
                #  getting all the params that will be used in GridSearch for the specific model
    ...
    def initiate_best_parameter_search_for_initialized_models(self,
        ...
        try:
            ...
            for initialized_model_list in initialized_model_list :
                ...
                #  getting the best params for the single single model
                ...
                # appeding all the best models in the list
    ...
    def get_best_model(self,X,y,base_accuracy = 0.6) -> BestModel :
        try :
            ...
            # getting the models with best grid search parameters
            ...
            # after getting the best models of GridSearch we will compare those models and get the model greater than the base accuracy
            return ModelFactory.get_best_model_from_grid_searched_best_model_list(grid_search_best_model_list,base_accuracy=base_accuracy)
        except Exception as e:
            raise HousingException(e,sys) from e 
